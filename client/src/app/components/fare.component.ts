...
stationForm = new FormGroup({
  ...
  processStnForm(){
    ...
    // need to find start value



    for(let i=0;i<this.stations.length;i++){
      if(this.stationForm.controls.startCtrl.value === this.stations[i].stn){
        var startIdx = this.stations[i].design;
      }
      if(this.stationForm.controls.endCtrl.value === this.stations[i].stn){
        var endIdx = this.stations[i].design;
      }
    }
    ...
    this.stnSvc.getFare(formObj)
    .then((result)=>{
      console.log(result);
      this.fare = result.message;
    })
    .catch((err)=>{
      console.log(err);
    })
