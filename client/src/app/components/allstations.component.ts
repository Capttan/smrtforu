...
@Component({
  ...
  styleUrls: ['./allstations.component.css']
})
export class AllstationsComponent implements OnInit {
  ...
  stationEWL = [
    ...
    ['Bugis',31,'BGS'],
    ['City Hall',11,'CTH'],
    ['Raffles Place',12,'RFP'],
    ['Tanjong Pagar',14,'TPG'],
    ['Outram Park',15,'OTP'],
    ['Tiong Bahru',16,'TIB'],
    ['Redhill',17,'RDH'],
    ['Queenstown',18,'QUE'],
  ...
  stationCCL = [
    ...
    ['Bayfront',229,'CBFT'],
    ['Promenade',213,'CPMN'],
    ['Nicoll Highway',212,'CNCH'],
    ['Stadium',211,'CSDM'],
    ['Mountbatten',210,'CMBT'],
    ['Dakota',209,'CDKT'],
    ['Paya Lebar',208,'CPYL'],
    ['MacPherson',207,'CMPS'],
    ['Tai Seng',206,'CTSG'],
    ['Bartley',205,'CBLY'],
    ['Serangoon',106,'CSER'],
    ['Lorong Chuan',203,'CLRC'],
    ['Bishan',3,'CBSH'],
    ['Marymount',205,'CMRM'],
    ['Caldecott',216,'CCDT'],
    ['Botanic Gardens',217,'CBTN'],
    ['Farrer Road',218,'CFRR'],
    ['Holland Village',219,'CHLV'],
    ['Buona Vista',20,'CBNV'],
    ['one-north',221,'CONH'],
    ['Kent Ridge',222,'CKRG'],
    ['Haw Par Villa',223,'CHPV'],
    ['Pasir Panjang',224,'CPPJ'],
    ['Labrador Park',225,'CLBD'],
    ['Telok Blangah',226,'CTLB'],
    ['HarbourFront',116,'CHBF']
  ];


  stationForm = new FormGroup({
    stnCtrl: new FormControl('',Validators.required),
  });

  // stateCtrl = new FormControl('',Validators.required);
  filteredStn: Observable<Station[]>;

  constructor(private stnSvc: StationService, private router: Router) { 
    this.filteredStn = this.stationForm.controls.stnCtrl.valueChanges
    .pipe(
      startWith(''),
  ...
  processStnForm(){
    const value = this.stationForm.controls.stnCtrl.value;
    ...
    console.log(stnId);
    this.router.navigate(['/curstn',stnId]);
  }
